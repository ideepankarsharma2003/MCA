-- POSTGRES Queries
-- https://onecompiler.com/studio/postgresql/42mcugvzd
...
CREATE TABLE Sales_order_details (
    ...
    Quantity_order NUMERIC(8),
    Quantity_disp NUMERIC(8),
...
SELECT p.Product_no, p.Description, sod.Quantity_order, TO_CHAR(so.Dely_date, 'Month') as Delivery_Month
FROM Product_master p
JOIN Sales_order_details sod ON p.Product_no = sod.Product_no
JOIN Sales_order so ON so.Order_no = sod.Order_no
...
SELECT Product_no, Description, Qty_on_hand, Reorder_lvl
FROM Product_master
...
SELECT DISTINCT c.Name
,TO_CHAR(so.Order_date, 'month yy') as Order_month
FROM Client_master c
JOIN Sales_order so ON c.Client_no = so.Client_no
WHERE so.Order_date < TO_DATE('may 14', 'month yy')
;
